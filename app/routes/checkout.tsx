...
export async function loader({ request, context }: Route.LoaderArgs) {
  ...
  const [orderResult, customerResult] = await Promise.allSettled([
    ...
    graphqlRequest<{ activeCustomer: ActiveCustomer | null }>(
      env,
      ACTIVE_CUSTOMER_QUERY,
      undefined,
      { request }
  ...
  const activeOrder =
    orderResult.status === "fulfilled" ? orderResult.value.data.activeOrder : null;
  const activeCustomer =
    customerResult.status === "fulfilled"
      ? customerResult.value.data.activeCustomer
      : null;
  const vendureBase = (
    env.VENDURE_SHOP_API ?? "http://localhost:3000/shop-api"
  ).replace("/shop-api", "");
...
function StepPanel({
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={isCompleted && !isActive && canOpen ? onOpen : undefined}
        className={`w-full flex items-center gap-4 p-5 text-left ${
          isCompleted && !isActive ? "cursor-pointer hover:bg-gray-50" : "cursor-default"
        }`}
      >
        <div
      ...
      {isActive && (
        <div className="px-5 pb-6 border-t border-gray-100 pt-4">{children}</div>
      )}
    </div>
  );
}
...
function CustomerStep({
  ...
  onComplete: (s: CustomerSummary) => void;
}) {
...
function ShippingMethodStep({
  ...
  isActive: boolean;
  currency: string;
  onComplete: (method: ShippingMethod, totals: UpdatedOrderTotals) => void;
}) {
  ...
  function handleContinue() {
    ...
    setError(null);
    saveFetcher.submit(
      { _intent: "setShippingMethod", shippingMethodId: selected },
  ...
  return (
    <div className="pt-2">
      ...
      {!loadingMethods && methods.length === 0 && !error && (
        <p className="text-gray-500 text-sm py-4">No shipping methods available.</p>
      )}
...
function PaymentStep({
  ...
  useEffect(() => {
    ...
    if (d.addPaymentToOrder) {
      ...
      if (r.__typename === "Order") {
        ...
        const msg =
          (r.paymentErrorMessage as string) ||
          (r.eligibilityCheckerMessage as string) ||
          (r.message as string) ||
  ...
  function handlePay() {
    ...
    setError(null);
    payFetcher.submit(
      { _intent: "addPayment", method: selected, metadata: {} },
      { method: "post", encType: "application/json", action: "/api/checkout" }
    );
  }
    ...
    <div className="pt-2">
      ...
      {!loadingMethods && methods.length === 0 && !error && (
        <p className="text-gray-500 text-sm py-4">
          No payment methods available. Make sure a shipping method has been selected.
        </p>
      )}
          ...
          {methods.map((m) => (
            ...
            >
              ...
              <div className="flex-1">
                <p className="font-medium text-gray-900">{m.name}</p>
                {m.eligibilityMessage && (
                  <p className="text-sm text-yellow-600 mt-0.5">{m.eligibilityMessage}</p>
                )}
              </div>
            </label>
          ))}
          ...
          return (
            <div key={line.id} className="flex gap-3 p-4">
              ...
              <p className="text-sm font-semibold text-gray-900 flex-shrink-0">
                {fmt(line.linePriceWithTax, order.currencyCode)}
              </p>
            </div>
          );
        })}
        ...
        <div className="flex justify-between text-sm text-gray-600">
          ...
          <span>
            {order.shippingWithTax > 0
              ? fmt(order.shippingWithTax, order.currencyCode)
              : "—"}
          </span>
        </div>
...
export default function CheckoutPage() {
  const { activeOrder: initialOrder, activeCustomer, vendureBase } =
  ...
  const [customerSummary, setCustomerSummary] = useState<string | null>(
    activeCustomer
      ? `${activeCustomer.firstName} ${activeCustomer.lastName} — ${activeCustomer.emailAddress}`
      : null
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <StepPanel
            ...
            canOpen={completed.includes(2)}
            onOpen={() => goTo(3)}
            summary={shippingSummary ?? undefined}
          >
            <ShippingMethodStep
              isActive={step === 3}
                ...
                setShippingSummary(
                  `${method.name} — ${
                    method.priceWithTax === 0
                      ? "Free"
                      : fmt(method.priceWithTax, order.currencyCode)
                  }`
                );
