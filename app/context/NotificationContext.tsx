...
function Toast({ item, onDismiss }: { item: NotificationItem; onDismiss: (id: number) => void }) {
	...
	return (
		<div className={`relative flex items-start gap-3 bg-white shadow-lg rounded overflow-hidden ps-4 pe-4 py-3 min-w-[300px] max-w-sm pointer-events-auto transition-all duration-300 ease-out ${visible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-10"}`}>
			{/* Leading colour bar */}
			...
			<button onClick={() => onDismiss(item.id)} aria-label={locale === "ar" ? "إغلاق" : "Dismiss"} className="text-gray-400 hover:text-gray-600 transition-colors cursor-pointer shrink-0 mt-0.5">
				<X size={14} />
			</button>
		</div>
	);
...
export function NotificationProvider({ children }: { children: React.ReactNode }) {
	...
	return (
		<NotificationContext.Provider value={{ notify }}>
			{children}
			<div role="status" aria-label={locale === "ar" ? "الإشعارات" : "Notifications"} className="fixed top-4 end-4 z-[200] flex flex-col gap-2 pointer-events-none">
