...
function useBundleOffers(productId: string) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchBundleOffers(productId).then((data) => { if (!cancelled) setBundles(data); });
...
function resolveBundleImage(preview: string, vendureBase: string): string {
  ...
  const path = preview.startsWith("/assets/") ? preview
    : preview.startsWith("/") ? `/assets${preview}`
    : `/assets/${preview}`;
  return `${base}${path}`;
}
...
function BundleItemCard({
  ...
  return (
    <div className={`relative flex-shrink-0 ${width} rounded-xl border bg-white overflow-hidden transition-all duration-200
      ${selected ? "border-primary shadow-sm" : "border-gray-200 opacity-60"}`}
    >
      {/* Checkbox — top left */}
      ...
      {item.required && (
        <div className="absolute top-2 right-2 z-10 text-[9px] font-bold text-primary bg-primary/10 px-1.5 py-0.5 rounded-full">
          Required
        </div>
      )}
      ...
      <Link to={`/products/${item.productSlug}`} className={`block ${imgSize} bg-gray-50 overflow-hidden`}>
        {item.featuredAsset ? (
          <img
            src={resolveBundleImage(item.featuredAsset.preview, vendureBase ?? "")}
      ...
      <div className="p-2">
        <Link
          ...
          className="text-[11px] font-semibold text-gray-900 hover:text-primary transition-colors line-clamp-2 leading-snug block mb-1"
        >
          {item.variantName}
        </Link>
...
function BundleCard({ bundle, triggerVariantId, compact = false, vendureBase }: { bundle: BundleOffer; triggerVariantId: string; compact?: boolean; vendureBase: string }) {
  ...
  const [selectedOptional, setSelectedOptional] = useState<Set<string>>(
    () => new Set(sortedItems.filter((i) => !i.required).map((i) => i.productVariantId))
  );
  ...
  return (
    <div className="bg-white overflow-hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-primary to-primary/70 px-3 py-2 flex items-center justify-between rounded-t-xl">
        ...
        <span className="bg-amber-400 text-gray-900 text-[10px] font-extrabold px-2 py-0.5 rounded-full">
          {discountLabel}
        </span>
      </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {sortedItems.map((item) => (
            <BundleItemCard
              ...
              onToggle={() => toggle(item.productVariantId)}
              compact={compact}
...
export default function ProductBundleOffers({ productId, triggerVariantId, placement, vendureBase }: Props) {
    ...
    return (
      <div className="mt-2 flex flex-col gap-3">
        {inlineBundles.map((bundle) => (
          <BundleCard key={bundle.id} bundle={bundle} triggerVariantId={triggerVariantId} compact vendureBase={vendureBase} />
        ))}
      </div>
    );
  }
    ...
    <div className="mt-4">
      <div className="flex flex-col gap-3">
        {belowBundles.map((bundle) => (
          <BundleCard key={bundle.id} bundle={bundle} triggerVariantId={triggerVariantId} vendureBase={vendureBase} />
        ))}
      </div>
    </div>
  );
