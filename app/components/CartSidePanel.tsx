	...
	return (
		<div className="flex items-center justify-between mt-2">
			<div className="flex items-center gap-1">
				<button onClick={() => adjust(line.quantity - 1)} disabled={line.quantity <= 1 || isBusy} aria-label="Decrease quantity" className="w-6 h-6 flex items-center justify-center rounded border border-gray-300 text-gray-600 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer">
					<Minus size={11} />
				</button>
				...
				<button onClick={() => adjust(line.quantity + 1)} disabled={isBusy} aria-label="Increase quantity" className="w-6 h-6 flex items-center justify-center rounded border border-gray-300 text-gray-600 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer">
					<Plus size={11} />
				</button>
			</div>
			<div className="flex flex-col items-end gap-0.5">
				<button onClick={remove} disabled={isBusy} aria-label={`Remove ${line.productVariant.product.name} from cart`} className="text-gray-400 hover:text-red-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer self-end">
				...
				{line.discountedLinePriceWithTax < line.linePriceWithTax ? (
					<>
						<span className="text-xs text-gray-400 line-through">{formatPrice(line.linePriceWithTax, currencyCode)}</span>
						<span className="text-sm font-bold text-green-600">{formatPrice(line.discountedLinePriceWithTax, currencyCode)}</span>
					</>
				) : (
...
export default function CartSidePanel({ isOpen, onClose }: CartSidePanelProps) {
	...
	return (
		<>
			...
			{/* Panel */}
			<div role="dialog" aria-modal="true" aria-label="Shopping cart" className={`fixed top-0 right-0 h-full w-full max-w-md bg-white z-50 shadow-xl flex flex-col transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
				{/* Header */}
							...
							{totalDiscounts > 0 && (
								<div className="flex items-center gap-2 bg-green-50 border border-green-100 rounded-lg px-3 py-2 mb-4">
									...
									<span className="text-xs text-green-700 font-medium">
										You save: <strong>{formatPrice(totalDiscounts, order?.currencyCode ?? "QAR")}</strong>
									</span>
								</div>
							)}
